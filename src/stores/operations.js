...
export function createStudioOperations({
  ...
  const hasWorkspaceOperations = computed(
    () =>
      [...projects.values()].some(isManagedProject) || startingWorkspaceKeys.size > 0 || stoppingWorkspaceKeys.size > 0,
  );
  const cliMaintenanceBlocked = computed(
    () => state.cliMutating || state.environmentChecking || state.javaSelecting || hasWorkspaceOperations.value,
  );
  ...
  async function runWorkspace(path = state.selectedWorkspace, target = RUN_TARGETS.webInternal) {
    ...
    try {
      ...
      if (target === RUN_TARGETS.cliSystem) {
        await invoke('open_soloncode_system_terminal', {
      ...
      const project = await invoke('start_soloncode', {
        workspace: targetWorkspace,
        mode: option.mode,
        javaExecutable: state.javaExecutablePath || null,
      });
  ...
  async function stopWorkspace(path, mode) {
    ...
    if (startingRun?.instanceId) {
      ...
      if (cancelledRunInstances.size > 100) cancelledRunInstances.delete(cancelledRunInstances.values().next().value);
    }
    try {
  ...
  function handleFailed(event) {
    ...
    startingWorkspaceKeys.delete(key);
    startingRuns.delete(key);
    for (const project of [...projects.values()]) if (project.workspace_key === key) removeProject(project.project_key);
