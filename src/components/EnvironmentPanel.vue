  ...
  <div class="environment-panel">
    <header class="environment-header">
      ...
      <button
        ...
        type="button"
        :disabled="studio.state.cliMutating || studio.state.environmentChecking || studio.state.javaSelecting"
      ...
      <section class="environment-item">
        ...
        <dl class="environment-versions">
          <div>
            <dt>{{ t('environment.currentVersion') }}</dt>
            <dd>
              {{ studio.state.cliVersion || t(studio.state.installed ? 'common.unknown' : 'common.notInstalled') }}
            </dd>
          </div>
          <div>
            <dt>{{ t('environment.latestVersion') }}</dt>
        ...
        <div class="environment-controls">
          <div class="environment-status">
            <span class="environment-detail-label">{{ t('environment.status') }}</span>
            <span class="environment-detail-value" :class="{ 'environment-update': studio.state.cliUpdateAvailable }">
              {{
          ...
          <div class="environment-actions">
            <button
              ...
              type="button"
              :disabled="
                studio.cliMaintenanceBlocked.value || (studio.state.installed && !studio.state.cliUpdateAvailable)
              "
        ...
        <div class="environment-item-main">
          <div>
            ...
            <!--
                        ...
                        -->
            <p
              v-if="studio.state.javaExecutablePath"
              class="environment-executable-path"
              :title="studio.state.javaExecutablePath"
            >
              {{ t('environment.javaExecutable') }}：{{ studio.state.javaExecutablePath }}
            </p>
          </div>
        </div>
        <dl class="environment-versions">
        ...
        <div class="environment-controls">
          ...
          <div class="environment-actions">
            <button
              class="environment-action primary"
              type="button"
              :disabled="studio.state.cliMutating || studio.state.javaSelecting"
              @click="studio.switchJavaExecutable"
            >
              <AppIcon name="switch-java" />
              <span>{{ t('environment.switchJavaVersion') }}</span>
            </button>
          </div>
        </div>
      </section>
