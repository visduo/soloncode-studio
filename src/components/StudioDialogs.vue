...
<template>
    ...
    <div v-if="studio.dialogs.remote" class="dialog-backdrop">
        <section class="dialog-panel" role="dialog">
            <h2>{{ studio.dialogForms.editingRemote ? "修改远程工作区地址" : "添加远程工作区" }}</h2>
            <div class="workspace-alias-form">
                <input
                    v-model="studio.dialogForms.remote"
                    class="workspace-alias-input"
                    type="url"
                    placeholder="例如：https://example.com"
                    @keyup.enter="studio.saveRemote" />
            </div>
            <div class="dialog-actions">
                <button class="dialog-btn" type="button" @click="studio.dialogs.remote = false">取消</button>
                <button class="dialog-btn primary" type="button" @click="studio.saveRemote">
    ...
    <div v-if="studio.dialogs.terminalSettings" class="dialog-backdrop">
        <section class="dialog-panel terminal-settings-panel" role="dialog">
            <h2>终端设置</h2>
            <form class="terminal-settings-form" @submit.prevent="saveTerminal">
            ...
            <div class="dialog-actions">
                ...
                <button class="dialog-btn" type="button" @click="studio.dialogs.terminalSettings = false">取消</button>
