...
export function normalizeAppPreferences(preferences) {
  ...
  const autoCheckUpdates =
    typeof preferences?.autoCheckUpdates === 'boolean'
      ...
      : DEFAULT_APP_PREFERENCES.autoCheckUpdates;
  return { defaultRunTarget, themeStyle, themeMode, locale, notificationsEnabled, autoCheckUpdates };
}
...
export function normalizeTerminalSettings(settings) {
  ...
  if (settings && typeof settings === 'object') {
    ...
    if (Number.isFinite(Number(settings.lineHeight))) next.lineHeight = clampNumber(Number(settings.lineHeight), 1, 2);
